...
### Scoring Methodology (Brodie-Only Version)

This Brodie version does not rely on the original fixed marketing-style weight table. Instead, it computes evidence-backed dimension scores and then applies a configurable weighting profile.

#### Default Scoring Dimensions

| Dimension | Default Weight | Description |
|-----------|---------------|-------------|
| Technical Foundation | 20% | Crawlability, indexability, canonicalization, redirects, render parity, metadata quality, internal link health, and general page health |
| AI Accessibility | 20% | Whether important content is accessible to AI and search crawlers, including robots behavior, llms.txt presence and quality, key content discoverability, and whether important answer content is actually reachable |
| Content Quality | 20% | Answer-first structure, heading clarity, useful definitions, FAQ patterns, tables, lists, freshness indicators, author signals, citation support, and intent alignment |
| Citability | 20% | How strong the site's passages are for extraction and citation, including self-containment, claim specificity, entity density, heading support, citation cues, list or table usefulness, context independence, and duplication risk |
| Structured Identity | 10% | Schema quality, entity consistency, sameAs coverage, and whether the site clearly communicates who it is, what it offers, and how its entities connect |
| Platform Readiness | 10% | How well the site is positioned for Google AI Overviews, traditional search enhancements, Perplexity-style citation retrieval, ChatGPT-style answer retrieval, and Claude-style summarization |

#### Scoring Modes (Weighting Profiles)
...
Content Quality: 20%
Citability: 20%
Structured Identity: 10%
Platform Readiness: 10%
```
Use for general GEO and SEO audits.

**2. Technical Remediation Mode**
```
Technical Foundation: 25%
AI Accessibility: 15%
Content Quality: 15%
Citability: 15%
Structured Identity: 15%
Platform Readiness: 15%
```
Use when the goal is to identify crawl, rendering, indexing, metadata, and structural fixes first.

**3. Content Optimization Mode**
```
Technical Foundation: 15%
AI Accessibility: 20%
Content Quality: 25%
Citability: 25%
Structured Identity: 5%
Platform Readiness: 10%
```
Use when the goal is to improve extractability, answer quality, and page usefulness for AI and search responses.
...
Content Quality: 20%
Citability: 15%
Structured Identity: 20%
Platform Readiness: 10%
```
Use when entity clarity, structured identity, and local business discoverability are especially important.

#### Scoring Requirements

**Each scoring dimension must return:**
- `score` (0-100)
- `confidence` (0-1, where 1 = high confidence)
- `supporting_findings` (array of evidence)
- `affected_pages` (array of page URLs)
- `recommended_fixes` (array of actions)

**The final overall score must return:**
- `overall_score` (0-100)
- `overall_confidence` (0-1)
- `blockers_count` (critical issues)
- `fast_wins_count` (quick fixes)
- `strategic_opportunities_count` (long-term improvements)

**Important Rules:**
- Do not restore the old "Brand Authority Signals" as a major weighted score pillar
- Do not allow any single analyzer to emit a final score without supporting findings
- If evidence is missing, inconsistent, or weak, lower the confidence
- Never hide uncertainty
- Each recommendation must map back to findings that influenced scoring dimensions
